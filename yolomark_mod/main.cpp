	...
	try
	{
		...
		do {
			...
			for (size_t k = 0; k < current_coord_vec.size(); ++k)
			{
					...
					(i.abs_rect.y + preview.rows) < y_end && (i.abs_rect.y + i.abs_rect.height + preview.rows) > y_end)
				{
					if (selected_id < 0) {
						...
						selected_id = k;
						//std::cout << "Selected ID: " << selected_id << "current_coord_vec[selected_id].id: " << current_coord_vec[selected_id].id << std::endl;
			...
			if (delete_selected) {
				...
				//std::cout << "current_coord_vec.begin() + selected_id: " << current_coord_vec[0].abs_rect << " + " << selected_id << std::endl;
